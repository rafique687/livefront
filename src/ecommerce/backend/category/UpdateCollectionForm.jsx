...
const UpdateCollectionForm = () => {
  ...
  useEffect(() => {
    const fetchCollection = async () => {
      try {
        const res = await axios.get(`http://192.168.9.115:5000/api/collectionRoutes/single/${id}`);
        const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://192.168.9.115:5000/api/collectionRoutes/update/${id}`, payload);

      setStatus({ message: 'Collection updated successfully!', error: false });

      // Optional redirect
      setTimeout(() => navigate('/dashboard/category/collection'), 1500);
    } catch (err) {
      console.error(err);
      setStatus({ message: 'Failed to update collection.', error: true });
    }
  };
